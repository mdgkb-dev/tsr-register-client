<template>
  ...
  <el-table :data="patient.headCircumference" style="width: 100%" class="table-shadow" header-row-class-name="header-style">
    ...
    <el-table-column label="Группа">
      <template #default="scope">
        {{
          scope.row.getHeadCircumferenceGroup(patient.birthDate, patient.isMale).color
            ? `${scope.row.getHeadCircumferenceGroup(patient.birthDate, patient.isMale).percentiles}, ${
                scope.row.getHeadCircumferenceGroup(patient.birthDate, patient.isMale).recomendation
              }`
            : scope.row.getHeadCircumferenceGroup(patient.birthDate, patient.isMale)
        }}
      </template>
    </el-table-column>
    ...
    <el-table-column v-if="isEditMode" width="50" fixed="right" align="center">
      <template #header>
        <el-button type="success" size="small" :icon="Plus" circle @click="patient.addHeadCircumference()" />
      </template>
      <template #default="scope">
        <TableButtonGroup :show-remove-button="true" @remove="RemoveFromClass(scope.$index, scope.row.headCircumference, scope.row.headCircumferenceForDelete)" />
      </template>
    </el-table-column>
  </el-table>
</template>
