...
export default class MkbForm extends Vue {
  ...
  removeCheckedDiagnosis(item: any): void {
    const checkedDiagnosis = this.checkedDiagnosis.filter((diagnosis: IPatientDiagnosis) => {
      return diagnosis.mkbDiagnosisId === item.id || diagnosis.mkbSubDiagnosisId === item.id;
    });
