...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getCalculationsResults = (): FormulaResult[] => {
      ...
      researchResults.value.forEach((rr: ResearchResult) => {
        props.research.questions.forEach((q: Question) => {
          if (q.code) {
            item[q.code] = rr.getAnswerByQuestionId(q.id as string)?.valueNumber as number;
          }
          console.log(item);
        });
        const res = props.research.formulas[0].getResult(item, false, 20);
        res.formulaName = 'формула';
        results.push(res);
      });
    ...
    onBeforeMount(async () => {
      ...
      data.value = {
        labels: [...researchResults.value.map((r: ResearchResult) => r.date)],
        datasets: [
          {
            label: 'Data One',
            backgroundColor: '#f87979',
            data: [...getCalculationsResults().map((fr: FormulaResult) => fr.value)],
          },
        ],
      };
      mounted.value = true;
