<template>
  <GridContainer grid-gap="5px" margin="10px" width="calc(100% - 20px)" grid-template-columns="repeat(auto-fit, minmax(280px, 1fr))">
    ...
    <GeneralItem
      v-for="(researchItem, i) in researches"
      :key="researchItem.id"
      :ready="$stringsService.formatToPercentage(patient.getResearchFillingPercentage(researchItem.id))"
      height="60px"
      @click="$emit('select', researchItem)"
    >
      <template #general-item> {{ i + 1 }}. {{ researchItem.name }}</template>
    </GeneralItem>
  </GridContainer>
</template>
